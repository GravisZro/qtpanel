...
void CDockedWindow::onWindowPropertyChanged(Atom atom)
{
  ...
  {
    //bool isUrgent = X11Core::getWindowUrgency(m_window); // blink the window... perhaps bring it to the front?
  }
}
...
//  _NET_WM_ACTION_CHANGE_DESKTOP
  ...
  if(state.contains(_NET_WM_STATE_MAXIMIZED_HORZ))
    menu.addAction("UnMaximize", this, SLOT(onUnMaximize()));
  else
...
void CDockedWindow::onUnMaximize(void)
{
  X11Core::unmaximizeWindow(m_window);
}
