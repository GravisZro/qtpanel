...
class XFreed <T*> // pointer specialized template
{
  public:
    ...
    template<typename V> inline V* operator= (V* input) { data = input; return input; }

    // will _always_ convert into another pointer
    template <typename V> inline operator V**(void) { return reinterpret_cast<V**>(&data); }
    template <typename V> inline operator V* (void) { return reinterpret_cast<V* >( data); }
...
template <Atom Atype>
Atom X11Core::getWindowProperty(const WId window, const Atom atom, unsigned char** data)
{
  static Atom retType;
...
Atom X11Core::getWindowProperty(const Atom Atype, const WId window, const Atom atom, unsigned char** data)
{
  static Atom retType;
  static int retFormat;
  static Atom bytesLeft;
...
void X11Core::sendNETWMMessage(const WId window,
                               ...
                               const long l4)
{
  XClientMessageEvent event = { ClientMessage, // type
                                0,        // serial
                                False,    // send_event
                                nullptr,  // display
                                window, // window
...
bool X11Core::makeSystemTray(const WId window)
{
  ...
  XClientMessageEvent event = { ClientMessage, // type
                                ...
                                32,       // format
                                { .l = {
                                        static_cast<long>(CurrentTime),
                                        static_cast<long>(_NET_SYSTEM_TRAY_SX),
                                        static_cast<long>(window), 0, 0 } }
                              };
...
{
  int numVisuals;
  XVisualInfo visualInfoTemplate = { nullptr,     // visual
                                     0,           // visualid
                                     QX11Info::appScreen(), // screen
                                     32,          // depth
                                     0,           // class
                                     0x00FF0000,  // red_maks
  ...
  XFreed<XVisualInfo*> visualInfoList = XGetVisualInfo(QX11Info::display(),
                                                         VisualScreenMask    |
                                                         VisualDepthMask     |
                                                         VisualRedMaskMask   |
                                                         VisualGreenMaskMask |
